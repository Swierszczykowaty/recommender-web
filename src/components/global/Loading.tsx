...
export default function Loading({ message }: LoadingProps) {
  ...
  return (
    <motion.div
      className="flex flex-col items-center justify-center gap-4 text-white text-base md:text-lg min-h-[40vh] pt-32 md:pt-44 px-4"
      ...
      aria-live="polite"
    >
      <div className="w-6 h-6 md:w-8 md:h-8 border-4 border-white/20 border-t-white border-r-white/10 border-b-white/40 border-l-white/70 rounded-full animate-spin" />
        ...
        {isSlow && (
          ...
          >
            <motion.p
              ...
              className="text-white/70 max-w-[48ch] text-xs md:text-sm"
            >
            ...
            <motion.p
              ...
              className="text-white/60 text-xs md:text-sm mb-2">
            ...
            {showButton && (
              ...
              >
                <Link href="/movies" onClick={handleNavigation} className="w-full sm:w-auto">
                  <motion.button 
                    whileTap={{ scale: 0.95 }}
                    className="w-full px-4 sm:px-6 py-2 text-sm md:text-base bg-white/10 border border-white/30 hover:bg-white/20 text-white cursor-pointer rounded-lg transition duration-300"
                  >
                    Browse Movies
                  </motion.button>
                </Link>
